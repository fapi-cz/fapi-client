...
class FapiClientOrdersTest extends TestCase
{
	...
	public function testCreateGetAndUpdateOrders(): void
	{
		...
		$foundOrder = $this->fapiClient->getOrders()->findAll([
			'invoice' => $createdOrder['invoice'],
			'limit' => 1,
			'sources' => ['form', 'api'],
		])[0];

		Assert::same($foundOrder['id'], $order['id']);
	}
